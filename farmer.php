		...
		<div class="main">
			...
			<aside>
				<div style="padding:10px;max-width:300px"><br>
					...
					<center>
						<div class="ltop"><img src="images/logo.png" height="100">
							<h4 style="color:#2E8B57">Smart Farm Solution System SFSS</h4><br>
						</div>
						<div style="padding:10px 5px;box-shadow:inset 0px 0px 4px #ccc;background:#F0FFFF">
							<h4 style="color:#4682b4;"><?php echo $usern; ?></h4>
			...
			<section>
				...
				<div class="mainactivity" style="padding:20px">
					<div style="max-width:450px;margin:0 auto">
						<br>
						<h3>Welcome <?php echo $usern; ?></h3><br>
						<h4 style='color:blue'>SFSS Product Demands</h4>
						<?php
						$data = "";
						$sql = mysqli_query($con, "SELECT *FROM `demand`");
						while ($row = mysqli_fetch_assoc($sql)) {
							$prod = prepare(ucwords($row['product']));
							$ms = prepare($row['measure']);
							$rate = fnum($row['rate']);
							$mxs = $row['maxsupply'];
							$st = $row['status'];
...
<script>
			...
			$.ajax({
				...
				if (res.trim() == "success") {
					setTimeout(function() {
						...
						if (code) {
							if (f == "saveitems") {
								saveitems(ids, u, code);
							} else {
								saveprods(ids, u, code);
							}
						}
					}, 400);
				} else {
	...
	function checkchats() {
		$.ajax({
			...
			url: "datasaver.php",
			data: {
				getcom: "f-<?php echo $sid; ?>"
			}
		}).done(function(res) {
			if (res.trim() > 0) {
				_("notar").innerHTML = '<button class="notbtn">' + res.trim() + '</button>';
			} else {
				_("notar").innerHTML = '';
			}
		});
	...
	function showps() {
		...
		if (x.type === "password") {
			_("sps").style.display = "none";
			_("hdps").style.display = "block";
			x.type = "text";
		} else {
			_("sps").style.display = "block";
			_("hdps").style.display = "none";
			x.type = "password";
		}
	}
