...
if (isset($_GET['sales'])) {
	$data = "";
	$sql = mysqli_query($con, "SELECT  `day` FROM `purchases` ORDER BY `time` DESC");
	while ($rw = mysqli_fetch_assoc($sql)) {
		...
		while ($row = mysqli_fetch_assoc($qry)) {
			$pic = $row['photo'];
			$item = ucfirst(prepare($row['item']));
			$desc = nl2br(html_entity_decode(prepare($row['details'])));
			$cost = fnum($row['cost']);
			$fid = $row['farmer'];
			$day = date("h:i a", $row['time']);
			$pd = $row['paid'];
...
if (isset($_GET['addprod'])) {
	...
	while ($row = mysqli_fetch_array($sql)) {
		$img = $row['photo'];
		$desc = nl2br(prepare(ucfirst($row['details'])));
		$id = $row['id'];
		$prod = ucwords(prepare($row['product']));
		$cost = fnum($row['cost']);
...
if (isset($_GET['pickups'])) {
	$data = "";
	$no = 0;
	$cos = array("Narok North", "Narok East", "Narok South", "Narok West", "Kilgoris", "Emurua Dikirr");
	$opts = "";
	foreach ($cos as $pl) {
	...
	while ($row = mysqli_fetch_assoc($sql)) {
		$stn = prepare(ucwords($row['station']));
		$county = prepare(ucwords($row['county']));
		$no += 1;
		$id = $row['id'];
...
if (isset($_GET['demand'])) {
	...
	while ($row = mysqli_fetch_assoc($sql)) {
		$prod = prepare(ucwords($row['product']));
		$ms = $row['measure'];
		$rate = $row['rate'];
		$mxs = $row['maxsupply'];
		$st = $row['status'];
...
if (isset($_GET['payments'])) {
	...
	while ($ro = mysqli_fetch_assoc($sq)) {
		...
		while ($rw = mysqli_fetch_assoc($sql)) {
			...
			while ($gr = mysqli_fetch_assoc($sqr)) {
				$totd += $gr['cost'];
			}
			...
			while ($row = mysqli_fetch_assoc($qry)) {
				...
				$tot = $row['total'];
				$st = $row['status'];
				$cond = ($st == 0) ? "Supply unverified" : "Verified";
				$tm = $row['time'];
				$day = date("d-m-Y, h:i a", $tm);
				$no += 1;
				$totp += $tot;
	...
	$data = "";
	$sql = mysqli_query($con, "SELECT `farmer` FROM `payments` ORDER BY `time` DESC");
	while ($rw = mysqli_fetch_assoc($sql)) {
		$fid = $rw['farmer'];
		$tr = "";
		$no = 1;
		...
		while ($row = mysqli_fetch_assoc($qry)) {
			$mode = $row['mode'];
			$code = $row['transaction'];
			$amnt = fnum($row['amount']);
			$no += 1;
			$day = date("d-m-Y, h:i a", $row['time']);
			$desc = nl2br(prepare($row['description']));
...
if (isset($_GET['inputs'])) {
	...
	while ($row = mysqli_fetch_assoc($sql)) {
		$name = prepare($row['name']);
		$det = nl2br(prepare($row['details']));
		$pic = $row['photo'];
		$id = $row['id'];
		$cost = fnum($row['cost']);
	...
	$tr = "";
	$tot = 0;
	$qry = mysqli_query($con, "SELECT `product`,`photo`,`cost`,`farmer` FROM `sales` ORDER BY `time` DESC");
	while ($row = mysqli_fetch_assoc($qry)) {
		$name = prepare($row['product']);
		$pic = $row['photo'];
		$cost = fnum($row['cost']);
		$fid = $row['farmer'];
...
?>
	<div style="max-width:550px;margin:0 auto;">
		...
		<form method="post" id="upfom" onsubmit="saveinput(event)">
			<table cellpadding="10">
				<tr>
					<td>Name of Input<br><input type="text" name="pname" required autofocus></td>
					<td>Photo<br><input type="file" name="pic" id="pic" accept="image/*" required></td>
				</tr>
				<tr valign="top">
					<td>Usage Instructions<br><textarea name="det" id="nmsg" style="height:70px" required></textarea></td>
					<td>Cost<br><input type="number" name="icost" required></td>
				</tr>
				<tr>
					<td>County to be used<br><select name="cnty"><?php echo $opts; ?></select></td>
					<td><br><button class="btn" style="float:right">Upload</button></td>
				</tr>
			</table>
		</form>
	</div>
...
if (isset($_GET['news'])) {
	...
	while ($row = mysqli_fetch_assoc($sql)) {
		$mssg = nl2br(prepare($row['message']));
		$loc = ucfirst($row['county']);
		$tm = $row['time'];
		$day = date("M d, h:i a", $tm);
		$sen = ucfirst(prepare($row['source']));
	...
	?>
		<div style="padding:10px;width:260px;margin:20px;">
			<form method="post" id="ufom" onsubmit="saveacc(event)">
				...
				Password<br>
				<div style="width:245px;border:1px solid grey;border-radius:3px;background:#fff">
					<input type="password" name="pass" id="upass" value="<?php echo $pass; ?>" style="width:215px;border:0px;" required>
					<p style="float:right;margin:7px"><i class="fa fa-eye" style="cursor:pointer" onclick="showps()" id="sps"></i><i class="fa fa-eye-slash" style="cursor:pointer;display:none" onclick="showps()" id="hdps"></i></p>
				</div>
				<br><button class="btn" style="float:right">SAVE</button>
			</form>
		</div>
<?php
	}
