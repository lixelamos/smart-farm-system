...
if (isset($_GET['products'])) {
	...
	while ($row = mysqli_fetch_assoc($sql)) {
		$name = prepare(ucfirst($row['product']));
...
if (isset($_GET['agronchats'])) {
	...
	while ($row = mysqli_fetch_assoc($sql)) {
		...
		if ($no > 0) {
			$cnd = "<button class='notbtn'style='background:green' >$no</button>";
		} else {
			$cnd = "";
		}
...
if (isset($_GET['produce'])) {
	...
	while ($row = mysqli_fetch_assoc($sql)) {
		...
		$tm = $row['time'];
		$day = date("M d, H:i", $tm);
		$paid = ($row['paid'] == 0) ? "Supply Unpaid" : "Paid on " . date("d-m-Y,h:i a", $row['paid']);
		$tot = fnum($row['total']);
		$qn = $row['total'] / $row['rate'];
		$prod = prepare(ucwords($prd));
	...
	if ($prods == "") {
		...
		while ($row = mysqli_fetch_assoc($sq)) {
			$ms = prepare(ucwords($row['measure']));
			$mxs = $row['maxsupply'];
			$cost = $row['rate'];
		}

		$qry = mysqli_query($con, "SELECT *FROM `farmers` WHERE `id`='$sid' ");
		$loc = mysqli_fetch_assoc($qry)['location'];
		$opts = "";
...
if (isset($_GET['payments'])) {
	...
	while ($row = mysqli_fetch_assoc($sql)) {
		$mod = prepare($row['mode']);
		$amnt = fnum($row['amount']);
		$desc = nl2br(ucfirst(prepare($row['description'])));
		$day = date("M d, Y - h:i a", $row['time']);
...
if (isset($_GET["myacc"])) {
	...
	while ($row = mysqli_fetch_array($sql)) {
		$name = ucwords(prepare($row['name']));
		$loc = $row['location'];
		$pass = prepare($row['password']);
		$fon = prepare($row['phone']);
	}
